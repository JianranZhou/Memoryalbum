  ...
  <div class="flex flex-col">
    <div class="main-grid">
      <div class="post-header">
            ...
            <template v-else-if="!loading && article.tags && article.tags.length > 0">
              <li   v-for="tag in article.tags" :key="tag.id">
                ...
                <span
                  ...
                  <router-link  :to="'/article-list/' + tag.tagId">
                    {{ tag.tagName }}
                  </router-link>
        ...
        <h1 v-if="article.memoryTitle" class="post-title text-white">
          {{ article.memoryTitle }}
        </h1>
        <ob-skeleton
          v-else
    ...
    <div class="main-grid" style='width: 100%;background-color: #fff;border-radius: 30px;padding: 30px'>
      <div>
        <!-- 分享按钮 -->
        <el-button style="position: absolute;right: 180px;top: 30px;width: 90px;" @click="share"
                   type="primary">Share</el-button>
        <el-button style="position: absolute;right: 60px;top: 30px;width: 90px;" @click="del"
                   type="danger">Delete</el-button>
        <template v-if="article.memoryContent">

          <div style="display: flex;">
            <div>
              <div
                style="border-bottom: 5px solid orangered;display: inline-block;padding-bottom: 10px;margin-bottom: 20PX;font-size: 22px">
                Memory lmage</div>
              <div style="width: 260px;height: 240px;"><img style="width: 100%;" :src="article.memoryCover" alt="">
              </div>
            </div>
          ...
          <div class="post-html" style='width: 100%'>
            <div style='font-size: 22px;border: 10px;font-weight: 700'>Description:</div>
            <div class="markdown-body" ref="articleRef" v-html="article.memoryContent" />
          </div>
        </template>
  ...
  setup() {
    ...
    const fetchArticle = () => {
      ...
      api.getArticeById(reactiveData.articleId).then(({ data }) => {
        ...
        commonStore.setHeaderImage(data.data.memoryCover)
        new Promise((resolve) => {
          data.data.memoryContent = markdownToHtml(data.data.memoryContent)
          resolve(data.data)
        }).then((article: any) => {
          reactiveData.article = article
          reactiveData.wordNum = Math.round(deleteHTMLTag(article.memoryContent).length / 100) / 10 + 'k'
          reactiveData.readTime = Math.round(deleteHTMLTag(article.memoryContent).length / 400) + 'mins'
          loading.value = false
          nextTick(() => {
            Prism.highlightAll()
            initTocbot()
        ...
        new Promise((resolve) => {
          data.data.preArticleCard.memoryContent = markdownToHtml(data.data.preArticleCard.memoryContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
        ...
        new Promise((resolve) => {
          data.data.nextArticleCard.memoryContent = markdownToHtml(data.data.nextArticleCard.memoryContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
    ...
    const share = () => {
      ...
      ElMessageBox.alert(window.location.href, 'share', {
    ...
    const del = () => {
      return ElMessageBox.confirm(
        `Are you sure delete this memory ?`,{
          cancelButtonClass:'ExitCancelBtn',
          confirmButtonClass:'ExitCancelBtn',
          confirmButtonText: 'OK',
          type: 'warning',
        }
      ).then(
        () => {
          api.deleteMemory(reactiveData.articleId).then(({ data }) => {
            router.push({ path: '/memorylist/'  })
          })
        }
      )
