<template>
  <div class="flex flex-col">
    <div class="post-header">
      <Breadcrumb style="font-size: 22px;" :current="t('menu.archives')" />
      <h1 class="post-title text-white uppercase">
        {{ t('menu.archives') }}
      </h1>
    </div>
    <div class="bg-ob-deep-800 px-14 py-16 rounded-2xl shadow-xl block min-h-screen">
      <ul class="timeline timeline-centered" style="position: relative;">
        <el-button @click="memory" type="primary" v-if='userInfo'
          style="position: absolute;top: -20px;right: 30px;width: 100px;">Add Memory</el-button>
          ...
          <li class="timeline-item" v-for="article in archive.articles" :key="article.id">
            ...
            <div class="timeline-content">
              <router-link :to="'/articles/' + article.id">
                <h3 class="timeline-title">{{ article.memoryTitle }}</h3>
              </router-link>
              <p>
                {{ article.memoryContent }}
              </p>
            </div>
          </li>
        </template>
      </ul>
      <Paginator :pageSize="pagination.size" :pageTotal="pagination.total" :page="pagination.current"
...
export default defineComponent({
  ...
  setup() {
        ...
        .then(({ data }) => {
          data.data.records.forEach((item: any) => {
            item.articles.forEach((article: any) => {
              article.memoryContent = markdownToHtml(article.memoryContent)
                .replace(/<\/?[^>]*>/g, '')
                .replace(/[|]*\n/, '')
                .replace(/&npsp;/gi, '')
            })
    ...
    return {
      ...
      pageChangeHanlder,
      pagination,
      archives: toRef(articleStore.$state, 'archives'), memory,
      t
    }
  }
})
