  ...
  setup() {
    onMounted(() => {
      if (!userStore.userInfo){
        ElMessageBox.alert('If you use all the features of the website, please log in first', 'TIP', {
    ...
    const fetchTopAndFeatured = () => {
      api.getTopAndFeaturedArticles().then(({ data }) => {
        /*data.data.topArticle.memoryContent = markdownToHtml(data.data.topArticle.memoryContent)
          .replace(/<\/?[^>]*>/g, '')
          .replace(/[|]*\n/, '')
          .replace(/&npsp;/gi, '')*/
